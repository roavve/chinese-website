...
(function () {
  ...
  function persist() {
    ...
    // let the cloud layer (if present + logged in) sync this change up
    if (window.Cloud && window.Cloud.onLocalChange) window.Cloud.onLocalChange(state);
  }
  ...
  var Store = {
    ...
    // current full state, for pushing to the cloud
    snapshot: function () { return state; },

    // overwrite local state from a remote blob WITHOUT re-triggering a push
    // (writes straight to localStorage, bypassing persist()'s cloud hook)
    replaceAll: function (obj) {
      state = deepMerge(DEFAULTS, obj || {});
      try { localStorage.setItem(KEY, JSON.stringify(state)); }
      catch (e) { console.warn("Store: replaceAll save failed", e); }
    },
