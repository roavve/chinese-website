...
(function () {
  ...
  function initAuth() {
    ...
    if (!window.Cloud || !Cloud.enabled) {
      // accounts not configured -> stay fully local, hide the button
      if (btn) btn.hidden = true;
      return;
    }
    btn.hidden = false;
    btn.addEventListener("click", function () {
      if (Cloud.user()) openAccountMenu(); else openAuthModal();
    });

    // reflect auth state in the button + trigger sync on login
    Cloud.onChange(function (user) {
      updateAuthButton(user);
      if (user) onLogin(user); else reroute();
    });
    Cloud.onStatus(showSyncStatus);
  }
  ...
  // On login: pull the cloud blob. If it exists, adopt it. If the account is
  // brand new (no row yet) but this device has local progress, seed the cloud.
  function onLogin(user) {
    ...
    Cloud.pull().then(function (remote) {
      ...
      if (remote && Object.keys(remote).length) {
        ...
        Cloud.push(Store.snapshot()); // first sign-in seeds remote from local
      }
      Cloud.endApply();
      applyTheme();
      reroute();
  ...
  function showSyncStatus(s) {
    if (!syncStatusEl) {
      syncStatusEl = el("div", { class: "sync-status" });
      document.body.appendChild(syncStatusEl);
    }
    var map = { saving: "Syncing…", saved: "Synced ✓", error: "Sync error — saved locally" };
    syncStatusEl.textContent = map[s] || "";
    syncStatusEl.className = "sync-status show " + s;
    clearTimeout(syncStatusEl._t);
    if (s !== "saving") {
      syncStatusEl._t = setTimeout(function () {
  ...
  function modalShell(inner) {
    var overlay = el("div", { class: "modal-overlay", onclick: function (e) {
      if (e.target === overlay) overlay.remove();
    } });
    var box = el("div", { class: "modal" }, inner);
  ...
  function openAuthModal() {
    ...
    function render() {
      var email = el("input", { class: "field", type: "email", placeholder: "email", autocomplete: "email" });
      var pass = el("input", { class: "field", type: "password", placeholder: "password (6+ characters)", autocomplete: "current-password" });
      var msg = el("p", { class: "auth-msg" });
      var submit = el("button", { class: "btn btn-primary full" },
        [mode === "in" ? "Sign in" : "Create account"]);

      function doAuth() {
        var e = email.value.trim(), p = pass.value;
        if (!e || !p) { msg.textContent = "Enter an email and password."; return; }
        submit.disabled = true; submit.textContent = "…";
        var op = mode === "in" ? Cloud.signIn(e, p) : Cloud.signUp(e, p);
        op.then(function () {
          overlay.remove(); // auth-change listener handles the rest
        }).catch(function (err) {
      ...
      pass.addEventListener("keydown", function (ev) { if (ev.key === "Enter") doAuth(); });

      var toggle = el("button", { class: "auth-toggle", onclick: function () {
        mode = mode === "in" ? "up" : "in";
        clear(overlay); overlay.remove(); openAuthModal();
      } }, [mode === "in" ? "New here? Create an account" : "Have an account? Sign in"]);

      return [
        el("div", { class: "modal-seal", text: "印" }),
        el("h2", { class: "modal-title", text: mode === "in" ? "Welcome back" : "Create your account" }),
        el("p", { class: "modal-sub", text: "Sync your progress across devices. Or close this and keep practicing as a guest." }),
        email, pass, submit, msg, toggle
      ];
    }

    overlay = modalShell(render());
    overlay.querySelector(".field").focus();
  }
  ...
  function openAccountMenu() {
    ...
    var overlay = modalShell([
      el("div", { class: "modal-seal", text: "印" }),
      el("h2", { class: "modal-title", text: "Signed in" }),
      el("p", { class: "modal-sub", text: user.email }),
      el("p", { class: "auth-msg ok", text: "Your progress is syncing to the cloud." }),
      el("button", { class: "btn btn-ghost full", onclick: function () {
        Cloud.signOut().then(function () {
          overlay.remove();
          updateAuthButton(null);
          // keep local copy; just stop syncing
        });
      } }, ["Sign out"])
    ]);
  }
  ...
  function friendlyAuthError(err) {
    ...
    if (/password should be|at least 6/i.test(m)) return "Password must be at least 6 characters.";
    if (/confirm|not confirmed/i.test(m)) return "Check your email to confirm the account first.";
    return m;
  }
